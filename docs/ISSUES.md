...
| ISS-31 | P1 | 独立 worker 拓扑尚未做生产级验证 | lease、并发上限、重试预算、故障恢复和幂等写入通过演练 | 已加 3 次 stale recovery 上限并完成本地 kill/restart E2 演练；生产拓扑与容量证据待完成 | [#2](https://github.com/betterkite/WorldLoom/issues/2) |
| ISS-32 | P1 | 正式 provider 的价格与预算值需要按部署合同填写 | 严格 preflight 通过，未知价格不被伪造 | 已加 90 天价格快照元数据门禁与 mock 预算停止测试；累计 guard 仅覆盖 CompileRun，其他 LLM 路径、账户/网关预算控制、批准的实际 limits 与严格 preflight 仍待完成 | [#3](https://github.com/betterkite/WorldLoom/issues/3) |
