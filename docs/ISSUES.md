...
| ISS-31 | P1 | 独立 worker 拓扑尚未做生产级验证 | lease、并发上限、重试预算、故障恢复和幂等写入通过演练 | 已加 3 次 stale recovery 上限并完成本地 kill/restart E2 演练；CI run [35903165178](https://github.com/betterkite/WorldLoom/actions/runs/35903165178) 验证两个不同 Compose worker 副本分别领取独立队列任务（每项 1 次尝试）；DB 并发 CAS 集成测试通过。该空 fixture 探针未执行 provider 索引；生产 HA、全局并发预算、容量/SLO 和完整故障演练仍待完成 | [#2](https://github.com/betterkite/WorldLoom/issues/2) |
